...
const enablePlugin = async (item) => {
  try {
    Notify.info("Enable plugin");
    const response = await doVerneMQAPICall(
      item.Node,
      "/api/v1/plugin/enable?--name=" + item.Plugin,
    );
  } catch (error) {
    Notify.error("Error enable plugin" + error);
  }
};
...
const disablePlugin = async (item) => {
  try {
    ...
    const response = await doVerneMQAPICall(
      item.Node,
      "/api/v1/plugin/disable?--name=" + item.Plugin,
    );
  } catch (error) {
    Notify.error("Error disable plugin" + error);
  }
};
...
const loadPluginDataFromRestService = async (NodeName) => {
  ...
  try {
    ...
    pluginDataSelected.value = transformTableToArray(
  ...
  <v-container fluid>
    <v-row>
      <v-col>
        ...
        <v-card
          title="Plugins"
          subtitle="Lists all Plugins of the selected node"
        >
          <v-data-table
          ...
          >
            <template v-slot:item.actions="{ item }">
              <BtnIconWithTooltip
                ...
                @click.once="enablePlugin(item)"
                myicon="mdi-play"
              ></BtnIconWithTooltip>
              <BtnIconWithTooltip
                tooltip="Disable Plugin"
                @click.once="disablePlugin(item)"
                myicon="mdi-stop"
              ></BtnIconWithTooltip>
            </template>
          </v-data-table>
