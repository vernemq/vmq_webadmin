...
const startNode = async (item) => {
  try {
    ...
    const response = await doVerneMQAPICall(item.Node, "/api/v1/node/start");
  } catch (error) {
...
const stopNode = async (item) => {
  try {
    ...
    const response = await doVerneMQAPICall(item.Node, "/api/v1/node/stop");
  } catch (error) {
...
const leaveCluster = async (item) => {
  try {
    ...
    const response = await doVerneMQAPICall(
      item.Node,
      "/api/v1/cluster/leave?node=" + item.Node + "&--kill_sessions",
    );
  } catch (error) {
        ...
        <v-card title="Nodes" subtitle="Shows all nodes of the cluster">
          ...
          >
            <template v-slot:item.actions="{ item }">
              ...
              ></BtnIconWithTooltip>
              <BtnIconWithTooltip
                tooltip="Leave Cluster"
                @click="leaveCluster(item)"
                myicon="mdi-location-exit"
              ></BtnIconWithTooltip>
            </template>
          </v-data-table>
        </v-card>
