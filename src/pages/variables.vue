...
import {doVerneMQAPICall, transformTableToArray, selectedNodeValid} from "@/verne-api-helper"
...
const loadSettingsDataFromRestService = async (NodeName) => {
  try {
    const responseListener = await doVerneMQAPICall(NodeName,'/api/v1/config/show?app=vmq_server');
    variablesDataSelected.value = transformTableToArray(responseListener.data, NodeName);
          ...
          <v-data-table
            ...
            <v-dialog v-model="dialogDetails" max-width="500px">
              <v-card v-model="detailText">
                ...
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn color="blue-darken-1" variant="text" @click="closeDetails()">Ok</v-btn>
                  <v-spacer></v-spacer>
                </v-card-actions>
              </v-card>
            </v-dialog>
            ...
            <template v-slot:item.actions="{ item }">
              <BtnIconWithTooltip tooltip="Edit" @click="editDetails(item)" myicon="mdi-pencil-outline"></BtnIconWithTooltip>
              <BtnIconWithTooltip tooltip="View Details" @click="viewDetails(item)" myicon="mdi-eye-outline"></BtnIconWithTooltip>
            </template>
          </v-data-table>
        </v-card>
      </v-col>
