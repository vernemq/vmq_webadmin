...
const srccliendid = ref<any[]>([]);
const srcuser = ref<any[]>([]);
...
const reauthorizeSession = async (item) => {
  try {
    ...
    const responseSession = await doVerneMQAPICall(
      item.Node,
      "/api/v1/session/reauthorize?username=" +
        item.user +
        "&client-id=" +
        item.client_id +
        "&--mountpoint=" +
...
const disconnectSession = async (item) => {
  try {
    ...
    const responseSession = await doVerneMQAPICall(item.Node, Path);
...
const loadSessionDataFromRestService = async (NodeName) => {
  ...
  try {
    const responseListener = await doVerneMQAPICall(
      NodeName,
      "/api/v1/session/show?--client_id&--user&--is_online&--offline_messages&--online_messages&--mountpoint&--peer_host&--peer_port&--queue_started_at&--session_started_at&--node=" +
        NodeName,
    );
    sessionDataSelected.value = transformTableToArray(
            ...
            <template v-slot:expanded-row="{ columns, item }">
              <tr>
                <td :colspan="columns.length">
                  Queue started at
                  {{ new Date(item.queue_started_at).toISOString() }}
                </td>
              </tr>
              <tr>
                <td :colspan="columns.length">
                  Session started at
                  {{ new Date(item.session_started_at).toISOString() }}
                </td>
              </tr>
            </template>
            <template v-slot:item.actions="{ item }">
              <BtnIconWithTooltip
                tooltip="Reauthorize Sessions"
                @click="reauthorizeSession(item)"
                myicon="mdi-arrow-u-left-top"
              ></BtnIconWithTooltip>
              <BtnIconWithTooltip
                tooltip="Disconnect Session"
                @click="disconnectSession(item)"
                myicon="mdi-arrow-split-vertical"
              ></BtnIconWithTooltip>
            </template>
