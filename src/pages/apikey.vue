...
const deleteKey = async (item) => {
  try {
    ...
    const responseListener = await doVerneMQAPICall(item.Node,'/api/v1/api-key/delete?key='+item.Key);
  } catch (error) {
...
const loadPApiKeyDataFromRestService = async (NodeName) => {
  try {
    const responseListener = await doVerneMQAPICall(NodeName,'/api/v1/api-key/show');
    apiKeyDataSelected.value = transformTableToArray(responseListener.data, NodeName);
  } catch (error) {
  ...
  <v-container fluid>
    <v-row>
      <v-col>
        ...
        <v-card title="API Keys" subtitle="Lists all API keys of the selected node">
          <v-data-table
            :headers="apikeyHeaders"
            :items="apiKeyDataSelected"
            item-value="name">
            <template v-slot:item.actions="{ item }">
              <BtnIconWithTooltip tooltip="Delete Key" @click="deleteKey(item)" myicon="mdi-delete"></BtnIconWithTooltip>
            </template>
          </v-data-table>
        </v-card>
      </v-col>
