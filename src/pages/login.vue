...
const login = async (NodeName) => {
  ...
  try {
    const response = await axios.get('/webuiapi/login', {headers: {'username': username.value, 'password': password.value}, validateStatus: () => true});
...
<template>
  <v-container>
    ...
    <v-btn @click="login" color="primary" block class="mt-2">Sign in</v-btn>
        </v-card>
      </v-col>
    </v-row>
  </v-container>
