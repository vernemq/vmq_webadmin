...
export function transformTableToArray<T>(
  tableData: { table: T[]; type: string },
  NodeName,
): T[] {
  if (tableData.type === "table" && Array.isArray(tableData.table)) {
    if (NodeName == null) return [...tableData.table];
...
export async function doVerneMQAPICall(node, path) {
  ...
  const response = await axios.get(combinedPath, {
